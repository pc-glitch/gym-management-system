<template>
  <div class="like-general">
    ...
    >
      <div
        class="icon"
        v-html="currentItem.svg"
        :style="{
          fill: isActive
            ? currentItem.color || 'var(--primary-color, #1e90ff)'
...
const processingData = async () => {
  // 深拷贝数据
  parentData.value = JSON.parse(JSON.stringify(props.parentData));
