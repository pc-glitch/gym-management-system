...
<template>
  <div class="eventinformation-detail-page">
    <div v-if="!eventinformation" class="loading-container">
    ...
    <div v-else>
      ...
      <div class="container">
        ...
        <div class="breadcrumb">
          ...
          {{ eventinformation.name || eventinformation.title || "详情" }}
        </div>
        ...
        <div class="product-card">
          ...
          <!-- 右侧信息 -->
          <div class="info-col">
            <h1>{{ eventinformation.name || eventinformation.title || "标题" }}</h1>

            <div v-if="hasValue(eventinformation.classification)" class="subtitle">
              {{ eventinformation.classification }}
            </div>
            <div v-else-if="hasValue(eventinformation.profile)" class="subtitle">
              {{ eventinformation.profile }}
            </div>
            ...
            <div
              class="price-panel"
              :class="{ visible: hasValue(eventinformation.price) }"
            >
              <span class="price-label">价格</span>
              <span class="price-symbol">￥</span>
              <span class="price-val">{{ eventinformation.price }}</span>
              <span v-if="hasValue(eventinformation.price)" class="old-price">
                ￥{{ (eventinformation.price * 1.2).toFixed(2) }}
              </span>
            </div>
            ...
            <div class="btn-group">
              ...
              <div v-if="hasValue(eventinformation.price)" class="buy-btns visible">
                
                
                <button class="btn btn-buy" @click="handleBuy">立即购买</button>
                
              </div>
              <div v-if="hasValue(eventinformation.name) && hasValue(eventinformation.theleveloftheevent)" class="buy-btns visible">
            ...
            <div class="extra-actions">
              <LikeGeneral
                v-if="eventinformation.id"
                :refid="eventinformation.id"
                :parentData="eventinformation"
              ...
              <WeiboSharing
                v-if="eventinformation.id"
                :item="eventinformation"
                :title="eventinformation.name || eventinformation.title"
              />
              <QQSharing
                v-if="eventinformation.id"
                :item="eventinformation"
        ...
        <div class="detail-tabs">
          ...
          <div class="tab-content" v-show="activeTab === 'specs'">
            <table class="param-table">
                  ...
                  <tr v-if="hasValue(eventinformation.name)">
                    <td>赛事名称</td>
                  ...
                  <tr v-if="hasValue(eventinformation.classify)">
                    <td>赛事类型</td>
                    <td>{{ eventinformation.classify }}</td>
                  </tr>
                
              
                
                  <tr v-if="hasValue(eventinformation.theleveloftheevent)">
                    <td>赛事级别</td>
                    <td>{{ eventinformation.theleveloftheevent }}</td>
                  </tr>
                
              
                
                  <tr v-if="hasValue(eventinformation.venue)">
                    <td>举办地点</td>
                    <td>{{ eventinformation.venue }}</td>
                  </tr>
                
              
                
                  <tr v-if="hasValue(eventinformation.holdingdate)">
                    <td>举办日期</td>
                    <td>{{ eventinformation.holdingdate }}</td>
                  </tr>
                
              
                
                  <tr v-if="hasValue(eventinformation.competitiontime)">
                    <td>比赛时间</td>
                    <td>{{ eventinformation.competitiontime }}</td>
                  </tr>
                
              
                
                  <tr v-if="hasValue(eventinformation.totheschedule)">
                    <td>赛程安排</td>
                    <td>{{ eventinformation.totheschedule }}</td>
                  </tr>
          ...
          <div class="tab-content" v-show="activeTab === 'details'">
            <div v-if="hasValue(eventinformation.details)">
              <span v-html="eventinformation.details"></span>
            </div>
            <img
              v-for="(img, index) in allPhotos"
              :key="index"
          ...
          <div class="tab-content" v-show="activeTab === 'reviews'">
            <CommentComponent v-if="eventinformation.id" :refid="eventinformation.id" />
          </div>
...
const eventinformation = ref(null);
const mainImage = ref("");
const allPhotos = ref([]);
// 默认先显示参数
...
onMounted(() => {
  getEventinformationDetails();
});

// 获取赛事信息详情
const getEventinformationDetails = async () => {
  ...
  try {
    let res = await requestApi.post(`/api/eventinformation/info`, { id: productId });
    if (res.code != 0) {
      MessageBox.error(res.msg || "获取赛事信息详情失败");
      return;
    }

    eventinformation.value = res.data;
    allPhotos.value = handleImg(res.data);
    mainImage.value = allPhotos.value.length > 0 ? allPhotos.value[0] : "";
  } catch (error) {
    console.error("获取赛事信息详情失败:", error);
    MessageBox.error("获取赛事信息详情失败");
  }
};
...
const handleBuy = () => {
  ...
  const arr = [eventinformation.value];
...
const handleSignUp = () => {
  ...
  // 初始化表单
  signUpForm.eventId = eventinformation.value.id || "";
  signUpForm.name = eventinformation.value.name || eventinformation.value.title || "";
...
const submitSignUp = async () => {
  ...
  try {
    ...
    const registrationData = {
      shenheid: signUpForm.eventId,
      name: signUpForm.name,
      userid: parsedUserData.userid,
      username: signUpForm.username,
      phone: signUpForm.phone,
      registrationinformation: signUpForm.registrationinformation || `报名参加赛事: ${signUpForm.name}`,
      photo: signUpForm.photo
    };
    
    const response = await requestApi.post("/api/eventregistration/save", registrationData);
    if (response.code == 0) {
