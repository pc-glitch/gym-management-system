...
<template>
  <div class="list-page-container">
    <el-container>
      ...
      <el-aside width="240px" class="category-aside" v-if="categories.length > 0">
        <h3 class="aside-title">活动类型</h3>
                ...
                <div class="product-info">
                  ...
                  <div class="product-attributes">
                        ...
                        <div v-if="product.name" class="attribute-item">
                          <span class="attribute-label">活动名称:</span>
                        ...
                        <div v-if="product.classify" class="attribute-item">
                          <span class="attribute-label">活动类型:</span>
                          <span class="attribute-value">{{ product.classify }}</span>
                        </div>
                      
                    
                      
                        <div v-if="product.activitytheme" class="attribute-item">
                          <span class="attribute-label">活动主题:</span>
                          <span class="attribute-value">{{ product.activitytheme }}</span>
                        </div>
                      
                    
                      
                        <div v-if="product.sponsoringdepartment" class="attribute-item">
                          <span class="attribute-label">主办部门:</span>
                          <span class="attribute-value">{{ product.sponsoringdepartment }}</span>
                        </div>
                      
                    
                      
                        <div v-if="product.activitytime" class="attribute-item">
                          <span class="attribute-label">活动时间:</span>
                          <span class="attribute-value">{{ product.activitytime }}</span>
                        </div>
                      
                    
                      
                        <div v-if="product.venue" class="attribute-item">
                          <span class="attribute-label">活动地点:</span>
                          <span class="attribute-value">{{ product.venue }}</span>
                        </div>
                      
                    
                      
                        <div v-if="product.activestate" class="attribute-item">
                          <span class="attribute-label">活动状态:</span>
                          <span class="attribute-value">{{ product.activestate }}</span>
                        </div>
...
const fetchCategories = async () => {
  try {
    ...
    if (response.code == 0) {
      ...
      MessageBox.error(response.msg || "获取分类失败");
    }
  } catch (error) {
    console.error("获取分类列表失败:", error);
    MessageBox.error("获取分类列表失败");
  }
};
...
const fetchProducts = async () => {
  ...
  try {
    ...
    if (selectedCategory.value && selectedCategory.value !== 'all') {
      params.classify = selectedCategory.value;
    } else {
    ...
    const response = await requestApi.post("/api/activityinformation/page", filteredParams);
    if (response.code == 0) {
