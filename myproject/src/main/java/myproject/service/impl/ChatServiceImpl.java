...
public class ChatServiceImpl implements ChatService {
	...
	public void send(JSONObject jm, ChannelHandlerContext ctx, Integer type, Integer contentType) {
		...
		if (type == 0) {
			// 拿到对方ID上下文
			long toId = Long.parseLong(jm.get("toId").toString());
			ChannelHandlerContext toUserCtx = Constants.onlineUserMap.get(Long.toString(toId));
			// 对方在线
			if (toUserCtx != null) {
				sendMessage(toUserCtx, jm.toJSONString());
			}
		} else if (type == 1) {
			if (jm.get("toId") != null && StrUtil.isNotBlank(jm.get("toId").toString())) {
				// 拿到对方ID上下文
				ChannelHandlerContext toUserCtx = Constants.onlineUserMap.get(jm.get("toId").toString());
				// 对方在线
				if (toUserCtx != null) {
					sendMessage(toUserCtx, jm.toJSONString());
				}
			} else {
				for (Entry<String, ChannelHandlerContext> stringChannelHandlerContextEntry : Constants.onlineUserMap.entrySet()) {
					ChannelHandlerContext toUserCtx = stringChannelHandlerContextEntry.getValue();
					// 对方在线
					if (toUserCtx != null) {
						sendMessage(toUserCtx, jm.toJSONString());
					}
