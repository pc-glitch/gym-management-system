...
<template>
  <div class="venueinformation-container">
    <!-- 场地信息 搜索表单 -->
    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="用途类型" prop="classify">
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="俱乐部名称" align="center" prop="clubname">
        <template #default="scope">
          <span class="ellipsis">{{ displayOrEmpty(scope.row.clubname) }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="用途类型" align="center" prop="classify">
        <template #default="scope">
      ...
      <el-table-column label="场地状态" align="center" prop="yardstate">
        <template #default="scope">
          <span class="ellipsis">{{ displayOrEmpty(scope.row.yardstate) }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="场地规格" align="center" prop="venuespecifications">
        <template #default="scope">
          <span class="ellipsis">{{ displayOrEmpty(scope.row.venuespecifications) }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label=" 配套设施" align="center" prop="supportingfacilities">
        <template #default="scope">
          <span class="ellipsis">{{ displayOrEmpty(scope.row.supportingfacilities) }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="服务信息" align="center" prop="serviceinformation">
        <template #default="scope">
          <span class="ellipsis">{{ displayOrEmpty(scope.row.serviceinformation) }}</span>
        </template>
      </el-table-column>
    ...
    <!-- 场地信息 分页 -->
    <div class="venueinformation-pagination-container">
      <el-pagination
        v-show="total > 0"
        :total="total"
        ...
        @current-change="getVenueinformationList"
      />
    </div>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12" >
            <el-form-item label="用途类型" prop="classify">
              ...
              <el-select v-model="form.classify" placeholder="请选择用途类型" @change="handleClassifyChange">
                
                <el-option
                  v-for="option in siteclassificationOptions"
          ...
          <el-col :span="12" >
            <el-form-item label="场地状态" prop="yardstate">
              
                
              <el-select v-model="form.yardstate" placeholder="请选择场地状态" @change="handleYardstateChange">
                
                
                <el-option label="开放" value="开放" />
                
                <el-option label="暂停开放" value="暂停开放" />
          ...
          <el-col :span="24" >
            <el-form-item label="场地规格" prop="venuespecifications">
              
              <el-input
                v-model="form.venuespecifications"
                :rows="5"
                type="textarea"
                placeholder="请输入场地规格"
              />
          ...
          <el-col :span="24" >
            <el-form-item label=" 配套设施" prop="supportingfacilities">
              ...
              <el-input
                v-model="form.supportingfacilities"
                :rows="5"
                type="textarea"
                placeholder="请输入 配套设施"
              />
          ...
          <el-col :span="24" >
            <el-form-item label="服务信息" prop="serviceinformation">
              ...
              <el-input
                v-model="form.serviceinformation"
                :rows="5"
                type="textarea"
                placeholder="请输入服务信息"
              />
    ...
    <el-dialog title="查看详情" v-model="viewOpen" width="700px" append-to-body>
      <div class="details-container">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
          <div class="detail-item">
            <div class="detail-label">俱乐部名称:</div>
            <div class="detail-value">
              
               {{ displayOrEmpty(viewForm.clubname) }}
          ...
          <el-col :span="12">
          <div class="detail-item">
            <div class="detail-label">用途类型:</div>
            <div class="detail-value">
          ...
          <el-col :span="12">
          <div class="detail-item">
            <div class="detail-label">场地状态:</div>
            <div class="detail-value">
              
               {{ displayOrEmpty(viewForm.yardstate) }}
          ...
          <el-col :span="24">
          <div class="detail-item">
            <div class="detail-label">场地规格:</div>
            <div class="detail-value">
              
               {{ displayOrEmpty(viewForm.venuespecifications) }}
          ...
          <div class="detail-item">
            ...
            <div class="detail-value">
               ...
               {{ displayOrEmpty(viewForm.supportingfacilities) }}
          ...
          <div class="detail-item">
            ...
            <div class="detail-value">
               ...
               {{ displayOrEmpty(viewForm.serviceinformation) }}
...
const siteclassificationOptions = ref([]);
...
async function getSiteclassificationOptions() {
  try {
    const response = await requestApi.post("/api/siteclassification/list");
    if (response.code == 0) {
      siteclassificationOptions.value = response.data;
    } else {
      Message.error(response.msg || "获取用途类型列表失败");
    }
  } catch (error) {
    console.error("获取用途类型列表时出错:", error);
    Message.error("请求用途类型列表失败");
  }
}
...
function handleClassifyChange(selectedValue) {
  ...
  const optionsRef = siteclassificationOptions;
...
/** 场地状态 change handler */
function handleYardstateChange(selectedValue) {
  let selectedOption = null;

  const staticOptions = '开放,暂停开放,维护中'.split(',');
...
/** 查询场地信息列表 */
async function getVenueinformationList() {
  loading.value = true;
  try {
    ...
    const response = await requestApi.post("/api/venueinformation/page", queryParams.value);
    if (response.code == 0) {
      dataList.value = response.data.list;
      total.value = parseInt(response.data.total);
    } else {
      Message.error(response.msg || "获取场地信息列表失败");
    }
  } catch (error) {
    console.error("获取场地信息列表时出错:", error);
    Message.error("请求场地信息列表失败");
  } finally {
    loading.value = false;
  }
}
...
async function submitAddForm() {
  try {
    const response = await requestApi.post("/api/venueinformation/save", form.value);
    if (response.code == 0) {
...
async function submitUpdateForm() {
  try {
    const response = await requestApi.post("/api/venueinformation/update", form.value);
    if (response.code == 0) {
...
function handleDelete(row) {
  ...
  const confirmMessage = row.id
    ? `是否确认删除ID为"${row.id}"的场地信息数据项？`
    : `是否确认删除选中的${itemIds.length}条场地信息数据项？`;
  ...
  ElMessageBox.confirm(confirmMessage, "警告", {
    ...
    .then(async () => {
      try {
        const response = await requestApi.post("/api/venueinformation/delete", itemIds);
        if (response.code == 0) {
...
onMounted(async () => {
  ...
  getSiteclassificationOptions();
  
});
</script>

<style scoped>
.venueinformation-container {
  padding: 20px;
...
.venueinformation-pagination-container {
  display: flex;
