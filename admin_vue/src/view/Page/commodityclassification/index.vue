...
<template>
  <div class="commodityclassification-container">
    <!-- 商品分类 搜索表单 -->
    <!--
    ...
    <!-- 商品分类 分页 -->
    <div class="commodityclassification-pagination-container">
      <el-pagination
        v-show="total > 0"
        :total="total"
        ...
        @current-change="getCommodityclassificationList"
      />
    </div>
...
/** 查询商品分类列表 */
async function getCommodityclassificationList() {
  loading.value = true;
  try {
    
    const response = await requestApi.post("/api/commodityclassification/page", queryParams.value);
    if (response.code == 0) {
    ...
    console.error("获取商品分类列表时出错:", error);
    Message.error("请求商品分类列表失败");
  } finally {
    loading.value = false;
  }
}
...
function handleQuery() {
  ...
  getCommodityclassificationList();
}
...
async function submitAddForm() {
  try {
    const response = await requestApi.post("/api/commodityclassification/save", form.value);
    if (response.code == 0) {
...
async function submitUpdateForm() {
  try {
    const response = await requestApi.post("/api/commodityclassification/update", form.value);
    if (response.code == 0) {
...
function handleDelete(row) {
  ...
  const confirmMessage = row.id
    ? `是否确认删除ID为"${row.id}"的商品分类数据项？`
    : `是否确认删除选中的${itemIds.length}条商品分类数据项？`;
  ...
  ElMessageBox.confirm(confirmMessage, "警告", {
    ...
    .then(async () => {
      try {
        const response = await requestApi.post("/api/commodityclassification/delete", itemIds);
        if (response.code == 0) {
...
<style scoped>
.commodityclassification-container {
  padding: 20px;
...
.commodityclassification-pagination-container {
  display: flex;
