...
<template>
  <div class="activitytype-container">
    <!-- 活动类型 搜索表单 -->
    ...
    <!-- 活动类型 分页 -->
    <div class="activitytype-pagination-container">
      <el-pagination
        v-show="total > 0"
        :total="total"
        ...
        @current-change="getActivitytypeList"
      />
    </div>
...
async function getActivitytypeList() {
  ...
  try {
    ...
    const response = await requestApi.post("/api/activitytype/page", queryParams.value);
    if (response.code == 0) {
...
async function submitAddForm() {
  try {
    const response = await requestApi.post("/api/activitytype/save", form.value);
    if (response.code == 0) {
...
async function submitUpdateForm() {
  try {
    const response = await requestApi.post("/api/activitytype/update", form.value);
    if (response.code == 0) {
...
function handleDelete(row) {
  ...
  const confirmMessage = row.id
    ? `是否确认删除ID为"${row.id}"的活动类型数据项？`
    : `是否确认删除选中的${itemIds.length}条活动类型数据项？`;
  ...
  ElMessageBox.confirm(confirmMessage, "警告", {
    ...
    .then(async () => {
      try {
        const response = await requestApi.post("/api/activitytype/delete", itemIds);
        if (response.code == 0) {
...
.activitytype-pagination-container {
  display: flex;
