        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="赛事名称" prop="name">
              ...
              <el-select v-model="form.name" placeholder="请选择赛事名称" @change="handleNameChange">
                
                <el-option
                  v-for="option in eventinformationOptions"
                  :key="option.id"
...
const eventinformationOptions = ref([]);
...
function handleNameChange(selectedValue) {
  ...
  const optionsRef = eventinformationOptions;
...
async function getEventinformationOptions() {
  try {
    const response = await requestApi.post("/api/eventinformation/list");
    if (response.code == 0) {
      eventinformationOptions.value = response.data;
    } else {
      Message.error(response.msg || "获取eventinformation列表失败");
    }
  } catch (error) {
    console.error(error);
    Message.error("请求eventinformation列表失败");
  }
}
...
async function getList() {
  loading.value = true;
  try {
    const response = await requestApi.post("/api/eventregistration/page", queryParams.value);
    if (response.code == 0) {
...
async function submitForm() {
  formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        if (form.value.id) {
          const response = await requestApi.post("/api/eventregistration/update", form.value);
          if (response.code == 0) {
          ...
          const response = await requestApi.post("/api/eventregistration/save", form.value);
          if (response.code == 0) {
...
function handleDelete(row) {
  ...
  ElMessageBox.confirm(confirmMessage, "警告", {
    ...
    .then(async () => {
      try {
        const response = await requestApi.post("/api/eventregistration/delete", itemIds);
        if (response.code == 0) {
    ...
    .then(async () => {
      ...
      updateData.shenhehuifu =
        targetStatus == "通过"
          ...
          : "报名审核未通过，请完善信息后重新提交。";
      try {
        const response = await requestApi.post("/api/eventregistration/update", updateData);
        if (response.code == 0) {
